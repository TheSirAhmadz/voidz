...
Console  ──(VOIDZ_WORKER_TOKEN)───────────▶ Worker
Worker   ──(per-instance VOIDZ_CORE_API_TOKEN)──▶ Voidz Core
...
- Cookies: `HttpOnly`, `SameSite=Lax`, `Secure` in production
  (`VOIDZ_COOKIE_SECURE=1`), 7-day TTL, max 20 sessions per user.
- **CSRF**: every mutating request must repeat the session token in
  `X-Voidz-CSRF` (synchronizer-token; the cookie is unreadable to other
...
- **Worker token is shared**: suitable for one operator's nodes; multi-tenant
  ...
  interface; see `TODO(per-node-tokens)` in `worker/voidz_worker/main.py`).
