...
requests require the `X-Voidz-CSRF` header set to the session cookie value
(the frontend does this automatically). Worker routes use the shared
`VOIDZ_WORKER_TOKEN` bearer. Core management routes use the per-instance
`VOIDZ_CORE_API_TOKEN` bearer.
...
Management (bearer `VOIDZ_CORE_API_TOKEN`):
