...
Voidz is a control-plane / data-plane system with three components:
...
```
   ...
   Browser ────────────▶│  frontend (SPA) ── Console API (FastAPI)   │
                        ...
                        └───────────────┬────────────────────────────┘
                                        ...
                                        ▼
                        ┌────────────────────────────────────────────┐
                        │  VOIDZ CORE instances (isolated per user)  │
...
Structured as one relay engine shared across transports:
...
| `relay/xhttp.py` | One xHTTP engine serving `/xhttp-siz10/*` (VLESS) and `/txhttp-siz10/*` (Trojan): packet-up with ordered seq replay, stream-up with AIMD adaptive drain, queue-fed downlink, idle reaper |
...
| `app.py` | Route assembly; management API guarded by `VOIDZ_CORE_API_TOKEN` bearer auth |
...
- **Auth** — GitHub OAuth code flow with single-use server-side state;
  ...
  cookies; mutations additionally require the `X-Voidz-CSRF` header (the
...
- **Auth** — single shared `VOIDZ_WORKER_TOKEN`; the worker never sees the
...
commit}` (env: `VOIDZ_VERSION`, `VOIDZ_BUILD`, `VOIDZ_COMMIT`). The Console
