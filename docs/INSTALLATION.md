...
1. **Clone and create databases**
   ...
   ```bash
   git clone <your-fork> voidz && cd voidz
   createdb voidz
   ```
...
3. **GitHub OAuth App** — callback `http://127.0.0.1:8080/auth/callback`
   (or your public URL). Set `VOIDZ_GITHUB_CLIENT_ID` / `..._SECRET`.
...
4. **Run** — see `deploy/scripts/dev.sh`, or set the environment variables
   ...
   ```bash
   # console
   (cd console/api && .venv/bin/python -m voidz_console)         # :8080
   # worker
   VOIDZ_CORE_PYTHON=$PWD/core/.venv/bin/python \
   VOIDZ_CORE_CWD=$PWD/core \
   (cd worker && .venv/bin/python -m voidz_worker)               # :9100
   ```
