// Voidz Console frontend utilities: API client with CSRF, toasts, formatting.
...
async function request(method, path, body) {
  ...
  if (csrfToken) headers["X-Voidz-CSRF"] = csrfToken;
...
export function toast(message, kind = "info", ms = 3800) {
  ...
  if (kind === "ok") sound.success();
  else if (kind === "err") sound.error();
}
