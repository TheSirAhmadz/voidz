  ...
  async render(root) {
    root.innerHTML = `
      <div class="page-head">
        <div>
          <h1>Dashboard</h1>
          <div class="sub">Your Voidz instances at a glance.</div>
        </div>
    ...
    async function load() {
      ...
      document.getElementById("stats").innerHTML = `
        <div class="stat reveal" style="animation-delay:0ms"><div class="label">Active instances</div><div class="value">${instances.length}</div></div>
        <div class="stat reveal" style="animation-delay:60ms"><div class="label">Running</div><div class="value" style="color:var(--green)">${running}</div></div>
        <div class="stat reveal" style="animation-delay:120ms"><div class="label">Stopped</div><div class="value">${stopped}</div></div>
        <div class="stat reveal" style="animation-delay:180ms"><div class="label">Failed</div><div class="value" style="color:${failed ? "var(--red)" : "inherit"}">${failed}</div></div>`;
      ...
      if (instances.length === 0) {
        list.innerHTML = `
          <div class="empty">
            ...
            <div>Deploy your first Voidz Core instance in under a minute.</div>
        ...
        list.querySelectorAll(".inst-card").forEach((card, idx) => {
    ...
    root.querySelector("#cta-create").addEventListener("click", () => sound.click());
