...
Voidz's "cloud" provider for Railway (https://railway.com). Railway's
...
* Additional public endpoints are port-forwards (TCP proxies) — not needed by
  Voidz Core v1 since every supported protocol (VLESS / Trojan / Shadowsocks
...
One Voidz instance = one Railway service in the Voidz project, deployed from
a container image (default ``ghcr.io/voidzsh/voidz-core:latest``), with a
...
log = get("runtime", "voidz.console.railway")

GRAPHQL_URL = "https://backboard.railway.app/graphql/v2"

# Railway deployment status -> Voidz deployment status
STATUS_MAP = {
    "QUEUED": "queued",
...
class RailwayProvider:
    def __init__(self, token: str, project_id: str, environment_id: str,
                 core_image: str = "ghcr.io/voidzsh/voidz-core:latest"):
        self.token = token
    ...
    def from_env(cls) -> "RailwayProvider":
        token = os.environ.get("VOIDZ_RAILWAY_TOKEN", "")
        if not token:
            raise RailwayError("VOIDZ_RAILWAY_TOKEN is not set")
        project = os.environ.get("VOIDZ_RAILWAY_PROJECT_ID") or os.environ.get("RAILWAY_PROJECT_ID", "")
        environment = (
            os.environ.get("VOIDZ_RAILWAY_ENVIRONMENT_ID") or os.environ.get("RAILWAY_ENVIRONMENT_ID", "")
        )
        if not project or not environment:
            raise RailwayError(
                "VOIDZ_RAILWAY_PROJECT_ID / VOIDZ_RAILWAY_ENVIRONMENT_ID must be set "
                "(they are auto-injected when the Console itself runs on Railway)"
            )
        image = os.environ.get("VOIDZ_CORE_IMAGE", "ghcr.io/voidzsh/voidz-core:latest")
        return cls(token, project, environment, image)

    @staticmethod
    def available() -> bool:
        return bool(
            os.environ.get("VOIDZ_RAILWAY_TOKEN")
            and (
                os.environ.get("VOIDZ_RAILWAY_PROJECT_ID")
                or os.environ.get("RAILWAY_PROJECT_ID")
            )
            and (
                os.environ.get("VOIDZ_RAILWAY_ENVIRONMENT_ID")
    ...
    async def _gql(self, query: str, variables: dict[str, Any] | None = None,
        ...
        if resp.status_code == 401:
            raise RailwayError("Railway token rejected (401) — check VOIDZ_RAILWAY_TOKEN")
    ...
    async def configure_service(self, service_id: str, core_api_token: str, public_host: str) -> None:
        ...
        variables = {
            "VOIDZ_CORE_API_TOKEN": core_api_token,
            "VOIDZ_STATE_PATH": "/data/state.json",
            "PORT": "8000",
        }
        if public_host:
            variables["VOIDZ_PUBLIC_HOST"] = public_host
