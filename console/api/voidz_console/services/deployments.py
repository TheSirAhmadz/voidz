...
Providers:
* ``railway`` — one Railway service per Voidz instance (per-instance public
  domain via Railway's API, WebSocket-capable, TLS at the edge). Selected
  automatically when VOIDZ_RAILWAY_TOKEN + project/environment are configured.
* ``local``   — Voidz Worker on a node running instances as containers
...
log = get("runtime", "voidz.console.deploy")
...
async def _pipeline_railway(pool: asyncpg.Pool, deployment_id: str, inst: dict,
    ...
    if not service_id:
        ...
        service_id = await provider.create_service(f"voidz-inst-{instance_id[:8]}")
        await pool.execute(
...
async def _pipeline_local(pool: asyncpg.Pool, deployment_id: str, inst: dict) -> None:
    ...
    await _set_deployment(pool, deployment_id, "building")
    await _log(pool, deployment_id, f"Preparing Voidz Core {inst['core_version']}…")
