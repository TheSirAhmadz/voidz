# Voidz Console — API + frontend (single service, the public endpoint)
FROM python:3.12-slim AS base

ENV PYTHONUNBUFFERED=1 \
    PYTHONDONTWRITEBYTECODE=1 \
    VOIDZ_CONSOLE_PORT=8080
...
COPY console/api/voidz_console ./api/voidz_console
COPY console/frontend ./frontend

RUN useradd --uid 10001 --shell /usr/sbin/nologin voidz \
    && mkdir -p /data && chown voidz:voidz /data
USER 10001
