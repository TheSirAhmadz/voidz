"""Instance drivers for the Voidz Worker.

Two drivers share one interface:

* ``DockerDriver``   — production. Runs Voidz Core in an isolated container:
...
Selection: ``VOIDZ_WORKER_DRIVER=docker|process`` (default: docker if the
...
log = get("runtime", "voidz.worker.driver")
...
class DockerDriver(BaseDriver):
    ...
    def __init__(self, ports: PortAllocator, data_root: Path, network: str = "voidz"):
    ...
    async def launch(self, spec: LaunchSpec) -> InstanceHandle:
        ...
        name = f"voidz-inst-{spec.instance_id[:12]}"
        image = f"voidz/core:{spec.core_version}"
        ...
        try:
            await self._run(
                "run", "-d",
                "--name", name,
                "--label", "voidz.instance=" + spec.instance_id,
                "--label", "voidz.managed-by=voidz-worker",
                # resource limits
                ...
                "-e", f"VOIDZ_CORE_API_TOKEN={spec.api_token}",
                "-e", f"VOIDZ_PUBLIC_HOST={spec.public_host}",
                "-e", "VOIDZ_STATE_PATH=/data/state.json",
                "-v", f"{data_dir}:/data",
                image,
            )
        except DriverError:
...
class ProcessDriver(BaseDriver):
    ...
    def __init__(self, ports: PortAllocator, data_root: Path, core_cmd: list[str] | None = None):
        ...
        self.core_cmd = core_cmd or [
            os.environ.get("VOIDZ_CORE_PYTHON", ".venv/bin/python"),
            "-m", "voidz_core",
        ]
    ...
    async def launch(self, spec: LaunchSpec) -> InstanceHandle:
        ...
        env = os.environ.copy()
        env.update({
            "VOIDZ_CORE_API_TOKEN": spec.api_token,
            "VOIDZ_STATE_PATH": str(data_dir / "state.json"),
            "VOIDZ_PUBLIC_HOST": spec.public_host,
            "VOIDZ_LOG_JSON": "1",
            "PORT": str(port),
        })
...
def select_driver(data_root: Path) -> tuple[BaseDriver, str]:
    ports = PortAllocator(
        int(os.environ.get("VOIDZ_WORKER_PORT_START", "19000")),
        int(os.environ.get("VOIDZ_WORKER_PORT_END", "19999")),
    )
    chosen = os.environ.get("VOIDZ_WORKER_DRIVER", "").strip().lower()
    if not chosen:
