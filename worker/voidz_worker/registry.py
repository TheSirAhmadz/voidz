"""Persistent instance registry for the Voidz Worker.
...
def _path() -> Path:
    base = Path(os.environ.get("VOIDZ_WORKER_DATA", "/data/instances"))
...
def save_instance(instance_id: str, data: dict) -> None:
    ...
    try:
        ...
        get("runtime", "voidz.worker.registry").warning("registry save failed: %s", exc)
