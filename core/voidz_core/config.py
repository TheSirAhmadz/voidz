"""Configuration system for Voidz Core.

Sources, in ascending priority:
  1. Defaults defined here.
  2. Optional TOML configuration file (``--config /path/to/voidz-core.toml`` or
     ``VOIDZ_CONFIG``). Only a handful of keys are honored; see ``FILE_KEYS``.
  3. Environment variables.
  4. CLI flags (parsed in ``__main__.py``).

The config intentionally contains only the options a runtime actually needs.
Secrets (``VOIDZ_CORE_API_TOKEN``) are never echoed in API responses or logs.
"""
...
def build_config(argv: list[str] | None = None) -> tuple[CoreConfig, argparse.Namespace]:
    parser = argparse.ArgumentParser(prog="voidz-core", description="Voidz Core runtime")
    parser.add_argument("--config", dest="config_file", default=os.environ.get("VOIDZ_CONFIG"))
    ...
    env_map = {
        "port": ("PORT", int),
        "host": ("VOIDZ_HOST", str),
        "api_token": ("VOIDZ_CORE_API_TOKEN", str),
        "state_path": ("VOIDZ_STATE_PATH", str),
        "log_level": ("VOIDZ_LOG_LEVEL", str),
        "log_json": ("VOIDZ_LOG_JSON", lambda v: v.lower() in ("1", "true", "yes")),
        "public_host": ("VOIDZ_PUBLIC_HOST", str),
        "relay_buf": ("VOIDZ_RELAY_BUF", int),
        "sock_buf": ("VOIDZ_SOCK_BUF", int),
        "write_high_water": ("VOIDZ_WRITE_HIGH_WATER", int),
        "ws_handshake_timeout": ("VOIDZ_WS_HANDSHAKE_TIMEOUT", float),
        "upstream_connect_timeout": ("VOIDZ_UPSTREAM_CONNECT_TIMEOUT", float),
    }
