...
async def trojan_ws_tunnel(ctx: RelayContext, ws: WebSocket) -> None:
    ...
    # first frame identifies the link.
    try:
    ...
    # Resolve the link by password hash: the trojan password is the link
    # UUID, hashed with SHA-224 on the wire (cached).
    link = None
    ...
    ip = ws_client_ip(ws)
    if not ctx.connections.device_slot_available(link.uuid, ip, link.max_devices, link.allowed_ips):
