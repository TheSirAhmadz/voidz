...
Common VLESS/Trojan machinery, parameterized by an explicit
``RelayContext`` instead of importing panel globals from main.py.
"""
...
log = get("network", "voidz.relay")

SOCK_BUF = 512 * 1024

# Adaptive quota batching (per-connection EWMA).
...
def tune_socket(writer: asyncio.StreamWriter, sock_buf: int = SOCK_BUF) -> None:
    """TCP_NODELAY + OS socket buffers to shave latency."""
    try:
