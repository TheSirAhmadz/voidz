...
DSN = os.environ.get("VOIDZ_TEST_DSN", "postgresql://admin:voidz@127.0.0.1:5432/voidz")
CONSOLE = os.environ.get("VOIDZ_TEST_CONSOLE", "http://127.0.0.1:8080")
WORKER = os.environ.get("VOIDZ_TEST_WORKER", "http://127.0.0.1:9100")
WORKER_TOKEN = os.environ.get("VOIDZ_WORKER_TOKEN", "dev-worker-token-0123456789abcdef")
...
async def test_deploy_and_relay():
    ...
    os.environ.setdefault("VOIDZ_DATABASE_URL", DSN)
    os.environ.setdefault("VOIDZ_WORKER_TOKEN", WORKER_TOKEN)
    os.environ.setdefault("VOIDZ_LOCAL_WORKER_URL", WORKER)
    from voidz_console.services import deployments
    ...
    async with websockets.connect(f"{CONSOLE.replace('http', 'ws', 1)}/i/{token}/ws/{uuid}") as ws:
        ...
        if b"HELLO-VOIDZ" not in resp:
            resp += bytearray(await asyncio.wait_for(ws.recv(), timeout=10))
    assert resp[:2] == b"\x00\x00", f"missing prefix: {bytes(resp[:20])!r}"
    assert b"HELLO-VOIDZ" in resp[2:], f"payload missing: {bytes(resp[2:120])!r}"
    srv.shutdown()
    await pool.close()
...
class _Hello(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"HELLO-VOIDZ")
